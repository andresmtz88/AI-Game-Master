...
Player message: ${message}`,
    ...
    if (
      memoryUpdate &&
      message &&
      /\b(destroy|destroyed|burn|burned|collapse|collapsed|ruin|ruined|shatter|shattered)\b/i.test(message)
    ) {
      memoryUpdate.majorChoice =
        memoryUpdate.majorChoice || message.trim();

      if (/destroy/i.test(message)) {
        memoryUpdate.worldConsequence =
          "The Observatory Tower was destroyed to stop the ritual from spreading.";
      } else {
        const cleanedMessage = message.trim().replace(/^I\s+/i, "");
