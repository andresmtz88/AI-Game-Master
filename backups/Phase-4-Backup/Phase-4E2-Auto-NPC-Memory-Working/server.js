...
- Only add npcUpdates when the player clearly meets, helps, trusts, threatens, harms, or makes an important agreement with a known NPC.
    ...
    if (memoryUpdate) {
      ...
      if (memoryUpdate.npcUpdates && memoryUpdate.npcUpdates.length) {
        ...
        for (const update of memoryUpdate.npcUpdates) {
          ...
          if (update.relationshipToParty) {
            npc.relationshipToParty = update.relationshipToParty;
          }
      ...
      if (changed) {
        ...
        console.log("AUTO MEMORY UPDATE:", memoryUpdate);
      }
    }
