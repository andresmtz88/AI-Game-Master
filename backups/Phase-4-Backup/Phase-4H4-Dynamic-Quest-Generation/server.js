...
Output:
{
  ...
  "majorChoice": "",
  "worldConsequence": "",
  "sideQuest": "Discover how to use or restore the starlight fragment.",
  "objectiveProgress": "",
  "advanceAct": false,
  "npcUpdates": []
}
...
function getNpcDialogueContext(npcs) {
  ...
  return npcList
    .map((npc) => {
      ...
      const relationship =
        String(npc.relationshipToParty || "").toLowerCase();

      const disposition =
        String(npc.disposition || "").toLowerCase();
   ...
   ${updatedWorldState.currentLocation || "Not established."}`
    ...
    if (command === "/campaign") {
      ...
      const sideQuests = memory.worldState.sideQuests?.length
        ? memory.worldState.sideQuests.map((item) => `- ${item}`).join("\n")
...
const memoryUpdate = await analyzeMemoryUpdate(message, memory, reply);
    ...
    if (memoryUpdate) {
      ...
      if (memoryUpdate.sideQuest) {
        worldState.sideQuests = [
          ...new Set([
            ...(worldState.sideQuests || []),
            memoryUpdate.sideQuest,
          ]),
        ];

        worldState.sessionNotes = [
          ...(worldState.sessionNotes || []),
          `Auto memory: side quest added - ${memoryUpdate.sideQuest}`,
        ];
