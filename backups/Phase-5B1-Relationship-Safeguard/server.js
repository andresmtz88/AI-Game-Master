...
Use npcRelationshipSignal when the player's action meaningfully affects an NPC's trust, loyalty, fear, resentment, gratitude, suspicion, respect, or emotional bond.
...
Do not use npcRelationshipSignal for ordinary conversation.
Only use it for meaningful emotional or relational moments.
...
Player Action:
"We risk our lives to protect Sera Brightwind from Velkor's assassin."
...
Output:
{
  "npcRelationshipSignal": {
    ...
    "type": "positive",
    "intensity": "major",
    "note": "The party risked their lives to protect Sera Brightwind from Velkor's assassin."
  }
}
...
function applyNpcRelationshipSignal(npc, signal) {
  const positivePath = [
    "not yet met",
    "acquaintance",
    "friendly",
  ...
  const intensity = String(signal.intensity || "minor").toLowerCase();
  ...
  const currentRelationship = String(
    npc.relationshipToParty || "acquaintance"
  ).toLowerCase();

  // Close friends require multiple betrayals
  if (
    currentRelationship === "close friend" &&
    type === "negative" &&
    steps > 1
  ) {
    steps = 1;
  ...
  // Prevent dramatic relationship reversals
  const currentRelationship = String(
    npc.relationshipToParty || "acquaintance"
  ).toLowerCase();
  ...
  let current = String(npc.relationshipToParty || "acquaintance").toLowerCase();

  if (type === "positive") {
    let index = positivePath.indexOf(current);

    if (index === -1) {
      index = positivePath.indexOf("acquaintance");
    }

    const nextIndex = Math.min(index + steps, positivePath.length - 1);
    npc.relationshipToParty = positivePath[nextIndex];

    if (intensity === "major") {
      npc.disposition = "deeply grateful";
    } else {
  ...
  if (type === "negative") {
    let index = negativePath.indexOf(current);

    if (index === -1) {
      index = negativePath.indexOf("acquaintance");
    }

    const nextIndex = Math.min(index + steps, negativePath.length - 1);
    npc.relationshipToParty = negativePath[nextIndex];

    if (intensity === "major") {
      npc.disposition = "betrayed";
    } else {
...
Location memory behavior:

Use recorded location memory to shape narration.

If a location has status destroyed, damaged, restored, fortified, corrupted, cleansed, abandoned, or rebuilt, describe it accordingly.

If a location has relatedNPCs, those NPCs may be emotionally, historically, or practically connected to that place.

Do not describe a location as intact if memory says it is destroyed.
Do not describe a location as ruined if memory says it is restored or rebuilt.

Use location history naturally, but do not recite the raw memory record.
...
const memoryUpdate = await analyzeMemoryUpdate(message, memory, reply);
    ...
    if (memoryUpdate) {
      ...
      // NPC Relationship Signal
      if (
        memoryUpdate.npcRelationshipSignal &&
        memoryUpdate.npcRelationshipSignal.npcName &&
        memoryUpdate.npcRelationshipSignal.type
      ) {
        const npcs = readJsonFile("npcs.json", { npcs: [] });
        ...
        const targetNpc = npcs.npcs.find((npc) => {
          return (
            npc.name.toLowerCase() ===
            memoryUpdate.npcRelationshipSignal.npcName.toLowerCase()
          );
        });

        if (targetNpc) {
          applyNpcRelationshipSignal(
            targetNpc,
            memoryUpdate.npcRelationshipSignal
          );

          if (memoryUpdate.npcRelationshipSignal.note) {
            targetNpc.relationshipHistory =
              targetNpc.relationshipHistory || [];

            targetNpc.relationshipHistory.push(
              memoryUpdate.npcRelationshipSignal.note
            );
    ...
    worldState.sessionNotes = [
      ...(worldState.sessionNotes || []),
      `Auto memory: NPC relationship signal applied - ${targetNpc.name}`,
    ];
